...
module uart_rx #(
    ...
    logic [1:0] r_state;  // state: IDLE START_BIT DATA_BITS STOP_BIT
    logic [3:0] r_bit_cnt;  // r_data_reg에 들어갈 index값
    logic [7:0] r_data_reg;  //rx포트로 부터 들어온 bit를 담을 그릇
    logic [15:0] r_baud_cnt;  // 651ns : 9600x16 오버샘플링(16) count변수
    logic r_baud_tick;  // 651ns 마다 tick 발생 
    logic [3:0] r_baud_tick_cnt;  // 16개 오버샘플링값 count
