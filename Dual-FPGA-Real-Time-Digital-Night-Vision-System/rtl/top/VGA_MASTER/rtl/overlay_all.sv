...
module radar (
    ...
    // [중요] 파이프라인 딜레이로 인해 출력 신호도 레지스터(ff)로 변경됨
    output logic dot_on,
    ...
    // =========================================================
    // 0. 삼각함수 ROM 및 센서 데이터 수신 (기존과 동일)
    // =========================================================
    logic [19:0] sin_cos_mem[0:90];  // 90도까지 91개
    initial begin
    ...
    // 선형 매핑: distance(0~63) -> 반경(80~320)
    // 선형 매핑: distance(0~255) -> 반경(80~320)
    assign dist_scale = sw_scale ? ((distance * 32'd241) >> 8) + 32'd80 : ((distance * 32'd241) >> 6) + 32'd80;

    // =========================================================
    // ★ WNS 완벽 해결: 4-Stage 타겟 좌표 계산 파이프라인
    // =========================================================
    ...
    // ---------------------------------------------------------
    // [Stage 2]: 2차 주 곱셈 (Variable x Variable -> DSP 블록 사용)
    // ---------------------------------------------------------
    always_ff @(posedge clk) begin
        if (reset) begin
            dist_valid_p2 <= 1'b0;
    ...
    // ---------------------------------------------------------
    // [Stage 3]: 비율 보정(*3), 시프트(>>), 그리고 중심점 이동(X/Y_CENTER)
    // ---------------------------------------------------------
    always_ff @(posedge clk) begin
        if (reset) begin
            dist_valid_p3 <= 1'b0;
            ...
            if (WIDE_MONITOR_FIX) begin
                // *3 은 내부적으로 (mult_x_p2 << 1) + mult_x_p2 의 덧셈기로 합성됨
    ...
    // =========================================================
    // ★ 파이프라인 1단계 (Stage 1): 기본 연산 및 절대값 계산
    // =========================================================
    ...
    // =========================================================
    // ★ 파이프라인 3단계 (Stage 3): 최종 판단 및 출력 (generate 배열 사용)
    // =========================================================
    ...
    // 타겟(점) 병렬 렌더링 (for문 제거, generate 도입)
    logic [180:0] dot_hit_reg;  // 파이프라인 레지스터 추가
    logic
        circle_on_pre, line_on_pre;  // 동기화를 위한 사전 레지스터
    ...
    generate
        for (i = 0; i <= 180; i++) begin : gen_dot_hit
            ...
            // 주의: 배열 비교는 현재 들어온 x_pixel_st2, y_pixel_st2 기준
            ...
            if (WIDE_MONITOR_FIX) begin
                ...
                // assign hit_cond = ((tx_abs == 11'd1 && ty_abs <= 11'd1));
            end else begin
    ...
    // =========================================================
    // ★ 파이프라인 4단계 (Stage 4): 181-input OR 병합 및 최종 플래그 출력
    // =========================================================
    always_ff @(posedge clk) begin
        // 이미 10ns 동안 자리를 잡은 레지스터들을 OR 연산하므로 타이밍 위반 없음!
        dot_on    <= |dot_hit_reg;
