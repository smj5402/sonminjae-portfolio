...
module top_VGA_OV7670 (
    ...
    // 클럭 위자드 및 카메라 컨트롤러, ISP (기존 코드 동일)
    ...
    async_fifo #(
        .DATA_WIDTH(8),
        .DEPTH(4096)  // 넉넉한 버퍼 사이즈 (2의 거듭제곱)
    ) U_ASYNC_FIFO (
        // [Write 도메인] ISP 측 (pclk 기준)
        .wr_clk(pclk),
        .wr_reset(reset),
        .wr_en(we & ~fifo_full),  // FIFO가 꽉 차지 않았을 때만 쓰기
        .wr_data(wData),  // ISP 처리 완료된 8비트 데이터
        .full(fifo_full),

        // [Read 도메인] SPI 측 (clk_100m 기준)
        .rd_clk  (clk_100m),
        .rd_reset(reset),
        .rd_en   (fifo_rd_en),
        .rd_data (fifo_rdata),
    ...
    // =========================================================================
    //  2. SPI Master 연결 및 제어 (Glue Logic)
    // =========================================================================
    logic spi_tx_ready;
    logic spi_start;
    logic spi_done;
    ...
    // [핵심 제어 로직] 
    // FIFO가 비어있지 않고(!fifo_empty), SPI가 보낼 준비가 되었다면(spi_tx_ready)
    // SPI 전송을 시작(spi_start)하고 동시에 FIFO에서 데이터를 하나 뽑습니다(fifo_rd_en).
    ...
    spi_master U_SPI_MASTER (
        ...
        .clk(clk_100m),
        .reset(reset),
        .start(spi_start),  // 전송 시작 펄스
        .tx_data(fifo_rdata),  // FIFO에서 갓 나온 데이터
        .tx_ready(spi_tx_ready),  // SPI Idle 상태 알림
        .rx_data(),  // 카메라 송신 전용이라면 rx는 비워둠
        .done(spi_done),
        .cpol(1'b0),  // SPI Mode 0 설정
        .cpha(1'b0),

        // 외부 핀 연결 (Top 모듈 밖으로 나감)
        .sclk(spi_sclk),
        .mosi(spi_mosi),
        .miso(spi_miso),
        .cs  (spi_cs)
