...
module BCD_Convert (
    ...
    // BCD 연산을 위한 20-bit 내부 레지스터
    // [19:16] 100의 자리, [15:12] 10의 자리, [11:8] 1의 자리, [7:0] 입력 데이터
    logic [19:0] bcd;
    integer i;

    always_comb begin
        // 1. 초기화: BCD 영역은 0으로, 하위 8비트에는 입력 데이터(data_in)를 넣음
        bcd = 20'b0;
        bcd[7:0] = data_in;

        // 2. 입력 데이터의 비트 수(8비트)만큼 반복 (Shift and Add 3)
        for (i = 0; i < 8; i = i + 1) begin

            // 3. Shift 하기 전에 각 BCD 자릿수가 5 이상인지 검사하여 3을 더함 (Add 3)
            // combinational 회로이므로 반드시 blocking assignment(=) 사용
            if (bcd[11:8] >= 5) begin
                bcd[11:8] = bcd[11:8] + 3;
            end
