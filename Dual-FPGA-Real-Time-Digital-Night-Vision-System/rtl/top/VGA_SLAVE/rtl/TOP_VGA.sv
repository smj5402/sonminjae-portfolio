...
module TOP_VGA (
    ...
    // slave_top: (SPI + FIFO) + Addr_gen이 통합된 모듈
    slave_top u_SPI_slave_top (
        ...
        .downstream_ready(1'b1),     // 항상 준비됨 상태로 고정
        .output_data     (wData),
        .rx_fifo_empty   (),         // Top에서는 사용하지 않음
        .waddr           (wAddr),
        .we              (we),
    ...
    ImgMemReader_upscaler U_FBuffReader (
        ...
        .addr(rAddr),
        .imgData(rData),
        .port_red  (w_port_red),   // 수정: top 모듈의 출력 포트에 직접 연결
